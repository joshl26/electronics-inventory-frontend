...
export default class BarChart1 extends React.Component {
  ...
  handleChangeData() {
    // mapValues applies changeNumberOfData to each top-level value in initialState
  ...
  render() {
    ...
    return (
      ...
      >
        <ResponsiveContainer>
          <BarChart width={400} height={400} data={currentData}>
            <XAxis dataKey="name" />
            <YAxis yAxisId="a" />
            ...
            <Bar yAxisId="a" dataKey="uv">
              {currentData.map((entry, index) => (
                <Cell key={`cell-a-${entry.name || index}`} />
              ))}
            </Bar>
            <Bar yAxisId="b" dataKey="pv" label>
              {currentData.map((entry, index) => (
                <Cell key={`cell-b-${entry.name || index}`} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
